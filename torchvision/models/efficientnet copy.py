...
__all__ = [ #wild import시 네임 스페이스 관리
    "EfficientNet",
...
@dataclass
class _MBConvConfig:    #MBConv 클래스의 설정을 저장하는 데이터 클래스 정의
    expand_ratio: float #확장 비율 
    kernel: int
    stride: int     #stride 설정
    input_channels: int #입력 채널 설정
    out_channels: int   #출력 채널 설정
    num_layers: int     #레이어 수 설정
    block: Callable[..., nn.Module]     #블록 목록

    @staticmethod
    def adjust_channels(channels: int, width_mult: float, min_value: Optional[int] = None) -> int:
        return _make_divisible(channels * width_mult, 8, min_value) #채널이 8로 나누어 떨어지도록 조정.

class MBConvConfig(_MBConvConfig):  #데이터 클래스 _MBConvConfig를 상속
    ...
    def __init__(
        ...
        input_channels = self.adjust_channels(input_channels, width_mult)   #입력 채널과 너비 계수의 곱을 8로 나누어 떨어지도록 조정
        out_channels = self.adjust_channels(out_channels, width_mult)       #출력 채널과 너비 계수의 곱을 8로 나누어 떨어지도록 조정
        num_layers = self.adjust_depth(num_layers, depth_mult)            #레이어 수와 깊이 계수의의 곱을 정수로 조정
        if block is None:   #block이 None면 
            block = MBConv  #MBConv클래스의 블록을 사용. 왜 default로 사용하지 않는 걸까요?
        super().__init__(expand_ratio, kernel, stride, input_channels, out_channels, num_layers, block) #각 인자를 attribute로 설정

        #채널 수를 8로 나누어 지도록 조정하는 이유는 2의 거듭제곱으로 조정하여 메모리 및 연산 최적화로 예상 된다

    @staticmethod
    def adjust_depth(num_layers: int, depth_mult: float):
        return int(math.ceil(num_layers * depth_mult))  #깊이와 계수의 곱을 올림
...
class EfficientNet(nn.Module):  
    def __init__(
        ...
        norm_layer: Optional[Callable[..., nn.Module]] = None,  #nn모듈을 사용하는 임의의 callable인자가 norm_layer로 전달될 수도, 아닐수도 있음음
        ...
        if not inverted_residual_setting:   #inverted_residual_setting이 없을 시 오류 발생
            raise ValueError("The inverted_residual_setting should not be empty")
        elif not (  
            isinstance(inverted_residual_setting, Sequence)     #inverted_residual_setting이 Sequence 타입이고
            and all([isinstance(s, _MBConvConfig) for s in inverted_residual_setting])  #모든 요소가 _MBConvConfing 타입이면
        ):
            raise TypeError("The inverted_residual_setting should be List[MBConvConfig]")   #MBConvConfig 타입의 요소를 가진 리스트라고 오류 발생

        if norm_layer is None:  #nomalization layer가 따로 전달되지 않았으면 BatchNorm2d 클래스를 사용, 왜 Default를 안쓸까요?
            norm_layer = nn.BatchNorm2d

        layers: List[nn.Module] = []    #각 layer는 nn.moudle의 인스턴스로 이루어진 리스트이다.
